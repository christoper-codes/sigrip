...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-light text-dark dark:bg-dark dark:text-light text-base md:text-lg">
        ...
        <flux:sidebar sticky collapsible stashable class="border-e overflow-hidden border-border bg-dark dark:bg-card">
            ...
            <flux:sidebar.nav variant="outline">
                ...
                <flux:sidebar.group
                    ...
                    :expanded="
                        ...
                        auth()->user()->metadata['notifications'] > 0 ||
                        auth()->user()->metadata['alerts'] > 0 ||
                        auth()->user()?->company?->getActiveTickets() > 0
                    "
                    ...
                    @can('viewDepartmentManager', auth()->user())
                        <flux:sidebar.item class="!py-5 cursor-pointer! text-neutral-300! dark:text-neutral-200!" icon="exclamation-triangle" :href="route('alert.index')" :current="request()->routeIs('alert.index')" wire:navigate>
                            ...
                            @if(auth()->user()->metadata['alerts'] > 0)
                                <div class="inline rounded-sm border border-special bg-special/10 text-center text-xs px-2 py-0.5 ml-1">
