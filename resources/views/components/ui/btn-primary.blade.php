...
--}}
@props(['href' => null, 'type' => 'button', 'adaptive' => false])
...
@if($href)
    <a
        href="{{ $href }}"
        @if($adaptive) :class="$flux.dark ? 'liquid-glass' : 'liquid-glass-light'" @endif
        {{ $attributes->except('class')->merge(['class' => $wrapperClass]) }}
    >
        <span
            class="{{ $labelClass }} {{ $attributes->get('class') }}"
            @if($adaptive) :class="$flux.dark ? 'text-white' : 'text-black'" @endif
        >{{ $slot }}</span>
    </a>
@else
    <button
