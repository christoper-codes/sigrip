    ...
    <div class="h-full w-full relative">
        <x-appearance.header>
            <div class="text-3xl leading-normal space-y-2">
                <p class="uppercase font-bold special-font">{{ __('Aplicaciones') }}</p>
                <p class="text-sm opacity-70">{{ __('Información y configuración de aplicaciones por departamento') }}</p>
            </div>
        </x-appearance.header>
        <div x-data="{ selectedTab: '{{ auth()->user()->can('viewDepartmentManager', auth()->user()) ? 'info' : 'details' }}' }" class="w-full">
