{{-- ============================================================
     ...
     liquid-glass system as the buttons — plain `dark:` variant via the
     .dark .liquid-glass-light rule in landing.css, no Alpine binding.
     ============================================================ --}}
...
<section id="como-funciona" class="w-full py-24 sm:py-32">
    <div class="mx-auto max-w-6xl px-6 sm:px-0">
        ...
        <div class="relative hidden h-165 lg:block">
            <div class="absolute left-1/2 top-8 h-145 w-267 -translate-x-1/2 select-none">
                <svg class="pointer-events-none absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 1068 560" fill="none" preserveAspectRatio="none">
                    <g class="text-black/15 dark:text-white/20" stroke="currentColor" stroke-width="4">
                        <path d="M296,85 C320,85 366,65 390,65" stroke-dasharray="6 7" style="animation: bn-flow 1.1s linear infinite;"></path>
                        <path d="M678,65 C712,65 748,85 772,85" stroke-dasharray="6 7" style="animation: bn-flow 1.1s linear infinite 0.15s;"></path>
                        <path d="M296,415 C320,415 366,409 390,409" stroke-dasharray="6 7" style="animation: bn-flow 1.1s linear infinite 0.3s;"></path>
                        <path d="M678,409 C712,409 748,415 772,415" stroke-dasharray="6 7" style="animation: bn-flow 1.1s linear infinite 0.45s;"></path>
                        <path d="M534,119 C534,197 534,276 534,354" stroke-dasharray="6 7" style="animation: bn-flow 1.1s linear infinite 0.6s;"></path>
                        <path d="M152,139 C152,213 152,287 152,360" stroke-dasharray="6 7" style="animation: bn-flow 1.1s linear infinite 0.75s;"></path>
                    </g>
                </svg>

                @foreach ($steps as $step)
                    <div class="absolute w-72" style="top: {{ $step['top'] }}px; left: {{ $step['left'] }}px;">
                        <div class="liquid-glass-light rounded-2xl p-7">
                            <div class="flex items-center gap-3 pb-3 border-b border-black/8 dark:border-white/10">
                                <svg class="size-12 shrink-0 text-dark dark:text-light" viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    {!! $step['icon'] !!}
                                </svg>
                                <span class="text-lg font-semibold text-dark dark:text-light">{{ $step['title'] }}</span>
                            </div>
                            <div class="flex flex-col gap-2 pt-3">
                                @foreach ($step['bullets'] as $bullet)
                                    <span class="flex items-center gap-2 text-base text-dark/60 dark:text-light/60">
            ...
            @foreach ($steps as $step)
                <div class="liquid-glass-light rounded-2xl p-7">
                    <div class="flex items-start gap-3">
                        <svg class="size-14 shrink-0 text-dark dark:text-light" viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            {!! $step['icon'] !!}
                        </svg>
                        <div class="flex-1">
                            <span class="block text-xl font-semibold leading-tight text-dark dark:text-light">{{ $step['title'] }}</span>
                            <div class="mt-2 flex flex-col gap-2">
                                @foreach ($step['bullets'] as $bullet)
                                    <span class="flex items-center gap-2.5 text-base text-dark/60 dark:text-light/60">
                                        <span aria-hidden="true" class="h-px w-3 shrink-0 bg-dark/30 dark:bg-light/30"></span>
                                        <span>{{ $bullet }}</span>
                                    </span>
                                @endforeach
